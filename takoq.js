...
function drawQuesMulti(qelem, question, report) {
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        if (options[i]["audio"]) {
            let atmp = document.createElement("audio");
            atmp.setAttribute("controls", "");
            atmp.innerHTML = "Browser No Audio Support";
            let stmp = document.createElement("source");
            stmp.src = options[i]["audio"];
            stmp.setAttribute("type", "audio/mpeg");
            atmp.appendChild(stmp);
            tmp2.appendChild(atmp);
        }
        if (options[i]["video"]) {
            let atmp = document.createElement("video");
            atmp.setAttribute("width", "400");
            atmp.setAttribute("controls", "");
            atmp.innerHTML = "Browser No Video Support";
            let stmp = document.createElement("source");
            stmp.src = options[i]["video"];
            stmp.setAttribute("type", "video/mp4");
            atmp.appendChild(stmp);
            tmp2.appendChild(atmp);
        }
...
function validatePage(survey, report) {
    ...
    if (qlist.length > 0) {
        console.log("Unfinished Mandatory Questions:");
        console.log(qlist);
        return false;
    }
    else {
